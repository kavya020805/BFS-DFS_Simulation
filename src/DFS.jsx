...
export default class DFS extends Component {
    ...
    drawHexes() {
        ...
        this.setState(
            ...
            this.depthFirstSearchCallback = () => this.depthFirstSearch(this.state.playerPosition)
        )
    }
    ...
    handleClick() {
        ...
        if (cameFrom[JSON.stringify(this.Hex(q, r, s))]) {
            this.setState(
                ...
                () => this.depthFirstSearch(this.state.playerPosition)
            );
        }
    }
    ...
    depthFirstSearch(playerPosition) {
        let stack = [playerPosition];
        let cameFrom = {}; 
        cameFrom[JSON.stringify(playerPosition)] = JSON.stringify(playerPosition);
        let visited = new Set();
        visited.add(JSON.stringify(playerPosition));
        let hexPathMap = this.state.hexPathMap || [];
        while (stack.length !== 0) {
            let current = stack.pop();
            let arr = this.getNeighbors(current);
            arr.map((l) => {
                if (!visited.has(JSON.stringify(l)) && hexPathMap.includes(JSON.stringify(l))) {
                    stack.push(l);
                    visited.add(JSON.stringify(l));
        ...
        return (
            <div className="BFS">
                <div className="bfs-header-container">
                    <h1 className="bfs-heading">DEPTH FIRST SEARCH</h1>
                    <div className="bfs-description">
                        <p><strong>How to use:</strong> Click different areas of the board to see how DFS algorithm explores the graph!</p>
                        <p><strong>How it works:</strong><br/>
                        The path is being calculated live by exploring as deep as possible along each branch before backtracking. The red arrows indicate the path that the algorithm is taking from node to node.<br/>
                        </p>
                    </div>
                </div>
